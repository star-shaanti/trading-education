...
export default function GuidePage() {
  ...
  const local = { ...guidesFr[id || ""], ...guidesFrExtended[id || ""] };
  ...
  return (
    <Section title={title || "Guide"} subtitle={lang === "fr" ? "Contenu détaillé" : "Detailed content"}>
      ...
      {/* Maillage interne : guides ↔ analyses / rapports / webinaires / compte */}
      <div className="mx-auto mt-12 max-w-3xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6">
        <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-3">
          {lang === "fr" ? "Pour aller plus loin" : "Go further"}
        </h2>
        <ul className="space-y-2 text-sm">
          <li>
            <Link href="/analyses" className="text-brand-primary hover:underline">
              {lang === "fr"
                ? "Analyses de marché approfondies (données banques centrales)"
                : "In-depth market analysis (central bank data)"}
            </Link>
          </li>
          <li>
            <Link href="/rapports" className="text-brand-primary hover:underline">
              {lang === "fr"
                ? "Rapports hebdomadaires en PDF (téléchargement gratuit)"
                : "Weekly PDF reports (free download)"}
            </Link>
          </li>
          <li>
            <Link href="/webinaires" className="text-brand-primary hover:underline">
              {lang === "fr"
